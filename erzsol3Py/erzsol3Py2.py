...
read_erzsol:                 read single erzsol output file
plot_prob_1D:               plot probability for 1D, z
plot_prob_2D:               plot probability for 2D, x-y
plot_prob_3D:               plot probability for 3D x-y-z
...
March 3, 2020
- Added functions to plot probability in space: plot_prob
...
def plot_prob_1D(x_space, nx, prob, x_s):

    x_array = np.linspace(x_space[0], x_space[1], nx+1)
    x_list = np.zeros((4,2))
    count=0
    for i in range(0, x_list.shape[0]):
        x_list[i, 0]=x_array[i]
        x_list[i, 1]=x_array[i+1]
    x_list[1::, 0] +=10
    print(x_list)
    plt.figure()
    for i, val in enumerate(prob):
        plt.axvline(x=x_list[i,0], alpha=val)
        plt.axvline(x=x_list[i,1], alpha=val)

    plt.axvline(x=x_array[-1], alpha=prob[i])
    #plt.axvline(x=x_array)
    plt.scatter(z_s, 0.5)
    plt.show()

def plot_prob_2D(x_space, y_space, nx, ny, prob, x_s, y_s):

    x_array = np.linspace(x_space[0], x_space[1], nx+1)
    y_array = np.linspace(y_space[0], y_space[1], ny+1)

    x_mesh, y_mesh = np.meshgrid(x_array, y_array, indexing='ij')
    #xv, yv = np.meshgrid(x, y, sparse=False, indexing='ij')
    pred = np.reshape(prob, (ny, nx)).T

    #print(x_array)
    #print(y_array)
    fig, ax = plt.gca(), plt.gca()

    im = ax.pcolormesh(x_mesh,y_mesh,pred)
...
def plot_prob_3D(cl_sp, predictions, hypocenter):
    pointss=[]
    edgess = []
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ...
    ax.set_zlim(1600,3400)
    #x_space = np.array([5700, 8300])
    #y_space = np.array([3700, 5900])
    #z_space = np.array([1600, 3400])
    for i, val in enumerate(prediction):
        cube_def = [
            (cl_sp[i][0][0], cl_sp[i][1][0], cl_sp[i][2][0]),
            (cl_sp[i][0][0], cl_sp[i][1][1], cl_sp[i][2][0]),
            (cl_sp[i][0][1], cl_sp[i][1][0], cl_sp[i][2][0]),
            (cl_sp[i][0][0], cl_sp[i][1][0], cl_sp[i][2][1])
        ]
        points, edges = draw_cube(cube_def)
        #print(i)
        #print(points)
        faces = Poly3DCollection(edges, linewidths=1)
        faces.set_facecolor((0,0,1,prediction[i]))
        ax.add_collection3d(faces)
        # Plot the points themselves to force the scaling of the axes
        ax.scatter(points[:,0], points[:,1], points[:,2], s=0)
        ax.scatter(x_s, y_s, z_s, color='r')
